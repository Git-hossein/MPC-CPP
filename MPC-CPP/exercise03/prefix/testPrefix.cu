...
__global__ void prefixSumKernelRow(int* input, int n) 
{
    ...
    if (i < n) {
        ...
        temp[threadIdx.x] = 0;  // Handle out-of-bound threads
    }
    __syncthreads(); 

    
    for (int stride = 1; stride < THREADS ; stride *= 2) {
        int index1 = (stride-1) + (threadIdx.x * 2 *stride);
    ...
    if (i < n) {
        input[i] = temp[threadIdx.x];
    }
}



__global__ void prefixSumKernelLastColumn(int* input, int n) 
{
    unsigned int i = blockDim.x * blockIdx.x + threadIdx.x;
    extern __shared__ int temp[];
    ...
    // Load input into shared memory
    if (i < n) {
        temp[threadIdx.x] = input[i*256 + 255];
    } else {
        temp[threadIdx.x] = 0;  // Handle out-of-bound threads
    }
    __syncthreads(); 
    ...
    if (i < n) {
        input[i*256 + 255] = temp[threadIdx.x];
    }
}


__global__ void shiftToRight(int* input, int* output)
{
    unsigned int i = blockDim.x * blockIdx.x + threadIdx.x;
    if(i == 0){
        output[i] = 0;
...
__global__ void spreadToPrefixSumRow(int* input, int n) 
{
    unsigned int i = blockDim.x * blockIdx.x + threadIdx.x;
    extern __shared__ int temp[];
...
__global__ void spreadToPrefixSumLastColumn(int* input, int n) 
{
    unsigned int i = blockDim.x * blockIdx.x + threadIdx.x;
    extern __shared__ int temp[];
    ...
    if (i < n) {
        input[i*256] = temp[threadIdx.x];
    }
}

__global__ void featureExtractor(int* dst, int* prefixSum)
{
    unsigned int i = blockDim.x * blockIdx.x + threadIdx.x;
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        //1.
        int numBlocks = (nPix + THREADS - 1) / THREADS;
        prefixSumKernelRow<<<numBlocks, THREADS, THREADS * sizeof(int)>>>(gpuFeatureImg, nPix);
        //2.
        prefixSumKernelLastColumn<<<1, THREADS, THREADS * sizeof(int)>>>(gpuFeatureImg, nPix);
        //3.
        shiftToRight<<<numBlocks, THREADS>>>(gpuFeatureImg, gpuPrefixSumShifted);
        //4.
        spreadToPrefixSumLastColumn<<<1, THREADS, THREADS * sizeof(int)>>>(gpuPrefixSumShifted, nPix+1);
        spreadToPrefixSumRow<<<numBlocks, THREADS, THREADS * sizeof(int)>>>(gpuPrefixSumShifted, nPix+1);
    
        //5. 
        featureExtractor<<<numBlocks, THREADS>>> (gpuFeatureList, gpuPrefixSumShifted);
        cudaMemcpy( &nFeatures, gpuPrefixSumShifted + 256*256, sizeof(int), cudaMemcpyDeviceToHost);
